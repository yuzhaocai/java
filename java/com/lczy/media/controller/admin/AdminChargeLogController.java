 ...
 */
@Controller
@RequestMapping(value = "/admin/chargeLog")
public class AdminChargeLogController {
	...
	public String index(Model model, ServletRequest request) {
		...
		// 只查询成功的充值记录
		searchParams.put("EQ_status", "CL_S_FINISHED");
		
		Page<ChargeLog> data= chargeLogService.find(searchParams, page, size, sort);
		
		model.addAttribute("data", data);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		
		return "admin/chargeLog/index";
	}
