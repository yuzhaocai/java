...
<body class="c-body">
	...
	<div class="main-panel">
		...
		<div class="pad pd15">
			<form id="form_editMedia" class="form-horizontal form-create"
				...
				action="${ctx}/member/media/weixin/save">
				<zy:token />
				...
				<div class="form-group form-group-sm">
					...
					<div class="col-xs-5 has-feedback">
						<input id="checkMediaAccount" type="text" class="form-control" name="account" value="${media.account}" >
						<input type="hidden" value="${media.account}" class="oldAccount">
					</div>
				</div>
				<div class="form-group form-group-sm">
					<label class="control-label col-xs-2"><span
					...
					<div class="col-xs-4 has-feedback">
						<select class="hide select-modal" name="industryType"
							id="" multiple="multiple" data-title="选择行业类型">
						...
						<div class="col-xs-5 has-feedback">
							<c:forEach var="weixinCategory"
								...
								<label class="radio-inline"><input
									<c:if test="${media.category eq weixinCategory.itemCode }">checked="checked"</c:if>
									type="radio" name="category" value="${weixinCategory.itemCode}"
									id="${weixinCategory.itemCode}">${weixinCategory.itemName}</label>
							</c:forEach>
							<input type="hidden" value="${media.category}" class="oldCategory">
						</div>
					</div>
				</div>
				...
				<div class="form-group form-group-sm">
					...
					<div class="col-xs-3 has-feedback ${hasError3 }">
						<input type="file" class="form-control" name="fansNumFile" value="${media.fansNumPic }">
						<tags:fieldError commandName="weixin" field="fansNumFile"/>
					</div>
					<div class="col-xs-1"><img height="30" alt="" src="<zy:fileServerUrl value="${media.fansNumPic }" />"></div>
				</div>
	...
	<%@include file="/WEB-INF/views/member/media/edit-media-script.jspf"%>
	<%@include
