...
$(function() {
	...
	$('#new-dic-btn').click(function( event ) {
		$.get('${ctx}/admin/dic/create?ajax', function(data) {
	...
	$('#mod-dic-btn').click(function( event ) {
		...
		if (node) {
	    $.get('${ctx}/admin/dic/update/' + node.id + '?ajax', function(data) {
	    	if ($('#dic-modal').length > 0) {
                $('#dic-modal').remove();//从DOM删除已有的对话框
            }
	        $('body').append(data);
	...
	$('#new-dicitem-btn').click(function( event ) {
		...
		if(dicNode) {
	    $.get('${ctx}/admin/dic/item/create?ajax',{dicId:dicNode.id}, function(data) {
	...
	$('#mod-dicitem-btn').click(function( event ) {
		...
		if (node) {
      $.get('${ctx}/admin/dic/item/update/' + node.id + '?ajax', function(data) {
        $('#dic-item-modal').remove();//从DOM删除已有的对话框
        $('body').append(data);
        common.showModal('#dic-item-modal');
      });
