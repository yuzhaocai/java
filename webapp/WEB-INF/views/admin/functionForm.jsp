...
<form id="form1" action="${ctx}/admin/func/save" method="post" class="form-horizontal">
    ...
    <c:if test="${!empty func.parent }">
    <input type="hidden" name="parent.id" value="${func.parent.id}"/>
	</c:if>
...
$('#form1').validate({
	rules: {
		funcName: {
			required: true
			, rangelength: [3, 20]
			, remote : '${ctx}/admin/func/checkName?oldName=' + oldName + '&pId=${func.parent.id}'
		},
		action : {
			required: true
			, rangelength: [2, 100]
...
$('#del_btn').on('click', function() {
    func.delFunction('${func.id}', '${func.funcName}');
});
