...
!function($) {
	...
	var footerTemp = ''
			...
			+ '    <button type="button" class="btn-u btn-u-red ok">确定</button>'
			+ '</div>';
...
!function($) {
	...
	RegionModal.prototype = {
		...
		checked: function(input) { // 选中地区
			...
			} else if( $li.has('.header') ) { //选择省份，禁用该省的城市
				var that = this;
	...
	RegionModal.prototype = {
		...
		setSelected: function() { //设置隐藏的select元素的option选项
			...
			if (this.checkedValue != null) {
				var $dataValue = this.checkedValue.value;
				var $dataLabel = this.checkedValue.label;
		...
		isSelected: function(value) {
			//common.log(value + ' -> ' + this.checkedMap[value]);
			return this.checkedValue.value != null ;
		},
		initStatus: function() { //初始化地区的选中状态
			var that = this;
			this.$modal.find('li input').each(function() {
		...
		delegate: function() {
			...
			//点击地区
			this.$modal.delegate('li input', 'click', function() {
				$(this).closest('li').toggleClass(c, this.checkedValue);
				if( this.checked ) {
					that.checked(this);
				} else {
					that.unchecked(this);
